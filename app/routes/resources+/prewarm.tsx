    ...
    if (targets.has('link-previews')) {
        ...
        const worker = async () => {
            while (true) {
                ...
                try {
                    const res = await fetch(`${origin}/resources/link-preview?url=${encodeURIComponent(link)}`, { headers: { Accept: 'application/json' } })
                    if (res.ok) previews++
                } catch {
                    // ignore
                }
        ...
        await Promise.all(Array.from({ length: Math.min(concurrency, RECENT_PUBLICATIONS.length) }, () => worker()))
    }

    const ms = Date.now() - start
    return data({ targets: Array.from(targets), compiled, total, previews, previewsTotal, ms })
}
